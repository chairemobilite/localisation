...
// Types for the routing time and distance data
type RoutingTimeDistancesByMode = Record<
    string,
    {
...
// Type for the routing time and distance data for each destination
type RoutingTimeDistances = Record<
    string,
    {
        _sequence?: number;
        resultsByMode?: RoutingTimeDistancesByMode;
    }
>;

// Type for the destination result object
...
// Helper function to capitalize mode name
const capitalizeMode = (mode: string): string => {
...
// Helper to get destination UUIDs ordered by their _sequence (for display)
const getDestinationUuidsInOrder = (routingTimeDistances: RoutingTimeDistances): string[] => {
    const entries = Object.entries(routingTimeDistances);
    if (entries.length === 0) {
        return [];
    }
    const sorted = [...entries].sort((a, b) => {
        const seqA = a[1]?._sequence;
        const seqB = b[1]?._sequence;
...
type TotalCostItemProps = {
    ...
    percentageOfIncome: string; // Percentage string
};
...
const TotalCostItem: React.FC<TotalCostItemProps> = ({ id, totalCost, percentageOfIncome }) => (
    <div id={id} className="value-item">
        ...
        {/* <div>({percentageOfIncome}% of net income)</div> */}
    </div>
);
...
const FrequentDestinationCard: React.FC<FrequentDestinationCardProps> = ({ result }) => (
    <div className="value-item">
        <div className="frequent-destination-mode">
            ...
            <span>{capitalizeMode(result.mode)}</span>
        </div>
        <div>
...
export const LocalisationResultsSection: React.FC<SectionProps> = (props: SectionProps) => {
    ...
    const firstAddressUuid = Object.keys(props.interview?.response?.addresses || {})[0]; // UUID of first address
    const firstAddress = props.interview?.response?.addresses?.[firstAddressUuid]; // Full object for first address
    ...
    const firstAddressHousingCostPercentageOfIncome = firstAddress?.monthlyCost?.housingCostPercentageOfIncome || '0'; // % of income spent on housing
    ...
    const secondAddressUuid = Object.keys(props.interview?.response?.addresses || {})[1]; // UUID of second address
    const secondAddress = props.interview?.response?.addresses?.[secondAddressUuid]; // Full object for second address
    // TODO: This field should be required
    ...
    const destinationUuidsInOrder = getDestinationUuidsInOrder(
        firstAddressRoutingTimeDistances as RoutingTimeDistances
    );

    // Ordered list of destination UUIDs
    const destinationsByUuid =
        (props.interview?.response as { destinations?: Record<string, { name?: string }> })?.destinations || {}; // All destinations keyed by UUID
            ...
            <section className="location-comparison">
                ...
                <section id="costs-section">
                    ...
                    <TotalCostItem
                        ...
                        percentageOfIncome={firstAddressHousingCostPercentageOfIncome}
                    />
                    <TotalCostItem
                        id="total-cost-item-2"
                        totalCost={secondAddressTotalCost}
                        percentageOfIncome={secondAddressHousingCostPercentageOfIncome}
                    />
                ...
                <section id="frequent-destinations-section">
                    ...
                    {destinationUuidsInOrder.map((destinationUuid) => {
                        // Get the destination name
                        const rawName: string | undefined = destinationsByUuid[destinationUuid]?.name;
                        const destinationNameForDisplay =
                            rawName && rawName.trim().length > 0 ? rawName : 'Destination';
